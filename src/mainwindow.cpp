...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    // ---------- Columna izquierda: lista de vectores ----------
    ...
    // ---------- Columna central: tabs con vista 2D y vista 3D ----------
    auto *tabs = new QTabWidget();

    canvas = new VectorCanvas();
    canvas->setVectores(&listaVectores);
    tabs->addTab(canvas, "Plano X-Z (2D)");
    ...
    // --- Grupo Crear/Modificar/Eliminar ---
    ...
    // ---------- Conexiones (reemplazan al switch) ----------
...
// Muestra/oculta campos segun la operacion elegida (mejora de UX, no existia en consola)
void MainWindow::operacionSeleccionada(int index) {
    bool necesitaEscalar = (index == 0);                 // mult. escalar
    bool necesitaDosVectores = (index == 1 || index == 2 || index == 3 ||
                                 index == 4 || index == 7); // suma, resta, punto, cruz, angulo
    spinEscalar->setVisible(necesitaEscalar);
    comboVector2->setVisible(necesitaDosVectores);
}

// Equivalente al switch(operacion) dentro del case 4
void MainWindow::calcularOperacion() {
    ...
    switch (op) {
        ...
        case 1: case 2: case 3: case 4: case 7: { // requieren v2
